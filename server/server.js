...
app.post("/boardAdd", (req, res) => {
  ...
  let sql =
    "INSERT INTO board_edit VALUES(NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,now());";
  db.query(
    sql,
    [
      board_name,
...
app.get("/board", (req, res) => {
  ...
  let sql =
    "select * from board_" + req.query.boardName + " order by idx desc;";
  db.query(sql, (err, result) => {
    if (err) {
      throw err;
    } else {
...
app.post("/write", (req, res) => {
  ...
  let sql =
    ...
    req.query.boardName +
    " values(null, ?, ?, ?, ?, ?, ?, now());";
  db.query(sql, [title, writer, passwd, contents, image, count], (err) => {
    if (err) {
      throw err;
    } else {
